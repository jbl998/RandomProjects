...
def TaskTwoUpperBound(M,K)->(_void):
    ...
    print('M =',M,', K =',K,'Upper bound with 95% confidence bands:', U_KM+(sigma_U*1.96/np.sqrt(M))*np.array([-1,0,1]))
...
def EEV(K)->(_void):
    ...
    prod.addConstr(x2 == 925)
    prod.addConstr(x3 == 500)
